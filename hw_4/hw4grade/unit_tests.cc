...
TEST_P(ComplexTests, Conjugate) {
    ...
    ASSERT_EQ(conj.real(), r);
    ASSERT_EQ(conj.imaginary(), -i);
};
...
TEST_P(ComplexOperatorTests, Multiply) {
    ...
    Complex mult = a * b;
    ASSERT_NEAR(mult.real(), r1*r2 - i1*i2, DBL_PRECISION);
    ASSERT_NEAR(mult.imaginary(), r1*i2 + i1*r2, DBL_PRECISION);
};
...
TEST_P(ComplexOperatorTests, Add) {
    ...
    Complex b = Complex(r2, i2);
    Complex add = a + b;
    ASSERT_NEAR(add.real(), r1+r2, DBL_PRECISION);
    ASSERT_NEAR(add.imaginary(), i1+i2, DBL_PRECISION);
};
